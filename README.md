...
4) Some keycaps are not standard sizes: caps lock is stepped; backspace, enter and '#/~' keys are 0.5u shorter than standard - these desicions were made to make the keyboard cleaner/more compact and so that the space bar is the only key that needs to have a stabiliser! The disadvantage is the keys can be more difficult to buy/re-use. 
